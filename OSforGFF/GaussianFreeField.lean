...
This file contains two OS axiom proofs for general Gaussian measures:

**OS-0 (Alternative proof via quadratic form):**
- `gaussian_satisfies_OS0`: Z[∑ᵢ zᵢJᵢ] = exp(-½ ∑ᵢⱼ zᵢzⱼ⟨Jᵢ, CJⱼ⟩) is entire in zᵢ
- The main OS0 proof used by `GFFmaster.lean` is in `OS0_GFF.lean` (holomorphic integral theorem)

**OS-2 (Euclidean Invariance):**
- `gaussian_satisfies_OS2`: Z[gf] = Z[f] when covariance is E(4)-invariant
- `CovarianceEuclideanInvariantℂ`: The covariance invariance condition

Other OS axioms are proved in their own files:
OS1 in `OS1_GFF.lean`, OS3 in `OS3_GFF.lean`, OS4 in `OS4_Clustering.lean`/`OS4_Ergodicity.lean`.
-/
...
The main proof used by `GFFmaster.lean` is in `OS0_GFF.lean` (holomorphic integral theorem).
-/
