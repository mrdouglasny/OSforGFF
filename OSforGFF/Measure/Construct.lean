...
Constructs the Gaussian Free Field measure μ on S'(ℝ^d) from the free covariance:
...
noncomputable def constructGaussianMeasureMinlos_free (m : ℝ) [Fact (0 < m)] [GFFPropagator d m] :
  ProbabilityMeasure (FieldConfiguration d) := by
  classical
...
theorem gaussianFreeField_pairing_memLp
  ...
  rwa [memLp_map_measure_iff (by fun_prop) (distributionPairingCLM_measurable φ).aemeasurable] at h_memLp
...
lemma gff_pairing_square_integrable
  ...
  rw [memLp_map_measure_iff (by fun_prop) (distributionPairingCLM_measurable φ).aemeasurable] at h_memL2
...
lemma freeCovarianceFormR_gaussian_cf_pd (m : ℝ) [Fact (0 < m)] [GFFPropagator d m] :
  ...
  have h_symm : ∀ f : SchwartzTestFunction d, freeCovarianceFormR m (-f) (-f) = freeCovarianceFormR m f f := by
    ...
    have h1 : -f = (-1 : ℝ) • f := (neg_one_smul ℝ f).symm
    rw [h1, freeCovarianceFormR_smul_left, freeCovarianceFormR_smul_right]; ring
  exact gaussian_positive_definite_bochner T (freeCovarianceFormR (d := d) m) h_eq h_symm

/-- The free covariance form as a MinlosAnalytic.CovarianceForm d structure. -/
...
theorem gaussianFreeField_free_centered (m : ℝ) [Fact (0 < m)] [GFFPropagator d m] :
    isCenteredGJ (gaussianFreeField_free (d := d) m) := by
  intro φ
  unfold GJMean
...
theorem gaussianFreeField_pairing_expSq_integrable
  ...
  have h_meas : AEMeasurable (⇑(distributionPairingCLM φ)) (gaussianFreeField_free m).toMeasure :=
