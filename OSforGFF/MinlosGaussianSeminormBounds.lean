...
If the (squared) covariance seminorm `f ↦ ‖T f‖²` is continuous and the topology on `E` is
generated by a family of seminorms `p`, then `f ↦ ‖T f‖` is bounded by a finite supremum of
...
/-- If `f ↦ ‖T f‖²` is continuous, then `f ↦ ‖T f‖` is continuous. -/
theorem continuous_norm_of_continuous_norm_sq (T : E →ₗ[ℝ] H)
    (h_sq : Continuous fun f : E => (‖T f‖ ^ 2 : ℝ)) :
    Continuous fun f : E => ‖T f‖ := by
  have h_sqrt : Continuous fun f : E => Real.sqrt (‖T f‖ ^ 2 : ℝ) :=
    (Real.continuous_sqrt.comp h_sq)
  simpa [Real.sqrt_sq_eq_abs, abs_of_nonneg (norm_nonneg _)] using h_sqrt

/-- If the topology on `E` is generated by a family of seminorms `p` and `‖T ·‖` is continuous,
...
theorem seminorm_norm_comp_le_finset_sup
    ...
    (T : E →ₗ[ℝ] H)
    (h_sq : Continuous fun f : E => (‖T f‖ ^ 2 : ℝ)) :
    ∃ s : Finset ι, ∃ C : ℝ≥0, C ≠ 0 ∧ (normSeminorm ℝ H).comp T ≤ C • s.sup p := by
  have hq : Continuous ((normSeminorm ℝ H).comp T) := by
    have : Continuous fun f : E => ((normSeminorm ℝ H).comp T) f := by
