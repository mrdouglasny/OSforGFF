...
This file derives the **mixed representation** of the free scalar field two-point function
(Wightman function / covariance), expressing it as an integral over spatial momenta **k**
with explicit exponential decay in time. Starting from the Schwinger (proper-time) representation,
we perform Gaussian integrals and Laplace transforms to reach the final form.

## Overview

For a test function `f` with support in the half-space `x⁰ ≤ 0` (ensuring causality/reflection
positivity), we show:

```
∫∫ f̄(x) Δ₊(x,y) f(y) dx dy = (1/(2π)^d) ∫∫∫ f̄(x) f(y) · [∫ e^(-ik⁰(x⁰+y⁰)) / (k⁰² + ω²) dk⁰] · e^(ik·(x-y)) dk dx dy
```

where `ω = √(|k|² + m²)` is the on-shell energy, and the integral representation makes manifest
the analytic structure (poles at ±iω) and exponential decay in time.

## Main Theorems

- **`bessel_bilinear_eq_mixed_representation`**: Establishes the mixed representation with
  exponential time decay:
  ```
  ∫∫ f̄(x) Δ₊(x,y) f(y) = (1/(2(2π)^(d-1))) ∫∫∫ f̄(x) f(y) · (1/ω) e^(-|x⁰+y⁰|ω) · e^(ik·(x-y)) dk dx dy
  ```

- **`bilinear_to_k0_inside`**: Final form with k⁰ integral explicit, showing the Feynman
  propagator structure in the time direction.

- **`bilinear_schwinger_eq_heatKernel`**: Connects the Schwinger (proper-time) representation
  to the heat kernel representation, the starting point for the derivation.

- **`heatKernel_bilinear_to_mixed_rep`**: Performs the Laplace transform in proper time `s`
  to obtain the mixed representation from the heat kernel form.

## Derivation Strategy

1. **Schwinger → Heat Kernel** (`bilinear_schwinger_eq_heatKernel`): Express the covariance
   as `∫₀^∞ e^(-sm²) · [heat kernel] ds`.

2. **Fourier Transform** (`heatKernel_bilinear_fourier_form`): Use Fourier representation of
   the heat kernel to introduce momentum space.

3. **Gaussian Integrals** (`gaussian_fourier_1d`): Evaluate Gaussian integrals in time and
   space separately using Mathlib's `fourierIntegral_gaussian`.

4. **Laplace Transform** (`s_integral_eval_complex`, `laplace_s_integral_with_norm`): Perform
   the Laplace transform `∫₀^∞ e^(-sω²) √(π/s) e^(-t²/(4s)) ds = (π/ω) e^(-ω|t|)` to obtain
   exponential time decay.
...
theorem schwinger_bilinear_integrable (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ) :
  ...
  have h_bound_integrable : Integrable bound μ := by
    ...
    --           = Cf * ‖f‖_{L¹} * (1/m²) * 1 < ∞
    --
    -- Use the axiom schwinger_bound_integrable
  ...
  have h_meas : AEStronglyMeasurable F μ := by
    ...
    · -- H(p.1, ‖Θ p.2.1 - p.2.2‖) : ℂ is AEStronglyMeasurable
      -- Use the axiom heatKernelPositionSpace_aestronglyMeasurable
...
/-- **THEOREM** (converted from axiom): Fubini swap for the Schwinger integrand.
...
/-- **THEOREM** (formerly axiom): The Bessel bilinear form equals the
    Schwinger heat kernel bilinear form.
    ...
    2. **Fubini swap**: Exchanging the s-integral with the x,y-integrals.
       Uses the elementary axiom `schwinger_bilinear_integrable`.
...
theorem bilinear_schwinger_eq_heatKernel (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ) :
  ...
  -- This uses the schwinger_fubini_swap axiom
  exact schwinger_fubini_swap m f

/-- **THEOREM** (converting axiom): Heat kernel bilinear form equals the mixed representation.
...
theorem heatKernel_bilinear_to_mixed_rep (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ)
  ...
  - `fubini_s_ksp_swap` (AXIOM): Fubini for the s ↔ k_sp swap (line 519)
  ...
  **Stage 7:** Verify normalization:
  ...
  -- Main dependencies: laplace_integral_half_power (AXIOM), fubini_s_ksp_swap (AXIOM)
  --
  -- Stage 4: Apply gaussian_fourier_1d to evaluate k₀ integral:
  --   For z₀ = -x₀ - y₀:
...
/-- **THEOREM**: The Bessel bilinear form equals the mixed representation form.
    ...
    **Proof outline** (directly at bilinear level, using axioms):
...
/-- **THEOREM** (formerly axiom): The Bessel covariance bilinear form equals the
    k₀-inside momentum form.
    ...
    **Proof sketch** (formalization uses textbook axioms for technical integral manipulation):
...
theorem bilinear_to_k0_inside (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ)
  ...
  -- Step 1: Apply the axiom to convert LHS to mixed representation
