...
parameters zᵢ. Uses Hartogs' theorem (ℂ-differentiable ⟹ analytic in finite dimension)
to reduce to showing the derivative–integral interchange (eq. 4.10 of the paper):

  ∂/∂zⱼ ∫ exp(i ∑ᵢ zᵢ ⟨φ, fᵢ⟩) dμ = ∫ i⟨φ, fⱼ⟩ · exp(i ∑ᵢ zᵢ ⟨φ, fᵢ⟩) dμ

The derivative bound |⟨φ, fⱼ⟩| · exp(−Im⟨φ, fᵢ⟩) is shown integrable via Fernique's
theorem (∫ exp(α⟨φ,f⟩²) dμ < ∞) and Hölder's inequality (L² × L² → L¹).

## Axiom

- `differentiable_analyticAt_finDim`: Hartogs' theorem in n dimensions.
...
These lemmas establish the preconditions needed to apply the holomorphic integral theorem
